...
// ==========================================
// 2. PROSES AKSI: HAPUS DATA & RESTOCK STOK
// ==========================================
...
if (isset($_POST['update_status'])) {
    ...
    $q_update = "UPDATE penyewaan SET status_pembayaran = '$status_pemb', status_sewa = '$status_sewa', sudah_dibayar = '$sudah_dibayar' WHERE id = '$id_sewa'";
...
while ($u = mysqli_fetch_assoc($q_u)) { $users_list[] = $u; }
...
while ($p = mysqli_fetch_assoc($q_p)) { $produk_list[] = $p; }
...
<body>
                            ...
                            <?php if ($result && mysqli_num_rows($result) > 0): ?>
                                <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                    <?php 
                                        ...
                                        $sewa_id = $row['id'];
                                        $rincian_items = [];
                                        $q_dtl = mysqli_query($conn, "SELECT dp.jumlah, p.nama_produk FROM detail_penyewaan dp JOIN produk p ON dp.produk_id = p.id WHERE dp.penyewaan_id = '$sewa_id'");
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <strong class="text-dark"><?= !empty($row['nama_pelanggan']) ? htmlspecialchars($row['nama_pelanggan']) : "Pelanggan Umum"; ?></strong>
                                        </td>
                                        <td>
                                            <span class="d-block fw-semibold text-wrap" style="max-width: 250px;">
                                                <?= !empty($rincian_items) ? implode("<br>", $rincian_items) : '<span class="text-muted italic">ID #' . $row['id'] . '</span>'; ?>
                                            </span>
                                        </td>
                                        <td>
                                            <small class="d-block text-muted"><i class="fa-regular fa-circle-check text-success me-1"></i>Ambil: <?= date('d/m/Y', strtotime($row['tanggal_sewa'])); ?></small>
                                            <small class="d-block text-muted"><i class="fa-regular fa-circle-xmark text-danger me-1"></i>Kembali: <?= date('d/m/Y', strtotime($row['tanggal_kembali'])); ?></small>
                                        </td>
                                        <td>
                                            ...
                                            <?php if (strtolower($row['status_pembayaran']) != 'lunas'): ?>
                                                <div class="small text-muted mt-1">Sisa: <span class="text-danger fw-bold">Rp <?= number_format($sisa_tampil, 0, ',', '.'); ?></span></div>
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                            <?php 
                                        ...
                                        <td class="text-center">
                                            <!-- Tombol Aksi -->
                                            <button type="button" class="btn btn-sm btn-light text-primary border me-1" data-bs-toggle="modal" data-bs-target="#modalBukti<?= $row['id']; ?>" title="Lihat Bukti">
                                                <i class="fa-solid fa-image"></i>
                                            </button>
                                            <button type="button" class="btn btn-sm btn-light text-warning border me-1" data-bs-toggle="modal" data-bs-target="#modalEdit<?= $row['id']; ?>" title="Edit Status">
                                                <i class="fa-regular fa-pen-to-square"></i>
                                            </button>
                                            <a href="?aksi=hapus&id=<?= $row['id']; ?>" class="btn btn-sm btn-light text-danger border" onclick="return confirm('Hapus transaksi <?= $row['nomor_pesanan']; ?>? Stok akan dikembalikan.')" title="Hapus">
                                ...
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="6" class="text-center py-4 text-muted">Belum ada data penyewaan.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
    ...
    <!-- ========================================== -->
    <!-- MODALS (DILETAKKAN DI LUAR TABEL & WRAPPER) -->
    <!-- ========================================== -->

    <?php 
    if ($result && mysqli_num_rows($result) > 0) {
        mysqli_data_seek($result, 0); // Reset pointer untuk loop modal
    ...
    ?>
                ...
                <div class="modal-content border-0 shadow">
                    ...
                    <div class="modal-body text-center p-4">
                        ...
                        <?php if (!empty($file_bukti) && file_exists($path_bukti)): ?>
                            <img src="<?= $path_bukti; ?>" class="img-fluid rounded shadow-sm" style="max-height: 400px;" alt="Bukti">
        ...
        <div class="modal fade modal-edit-sewa" id="modalEdit<?= $row['id']; ?>" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow">
                    <div class="modal-header">
                        <h6 class="modal-title fw-bold">Update Status - <?= $row['nomor_pesanan']; ?></h6>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <form method="POST" action="">
                            ...
                            <div class="p-3 bg-light rounded border mb-3 box-kalkulator <?= strtolower($row['status_pembayaran']) == 'lunas' ? 'd-none' : '' ?>">
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" name="sudah_dibayar" class="form-control val-dibayar" value="<?= $sdh_bayar; ?>" min="0" max="<?= $tot_harga; ?>" oninput="hitungSisaTagihan(this)" onkeyup="hitungSisaTagihan(this)">
                                    </div>
                                </div>
                                <div class="p-2 bg-white rounded border">
                                    <label class="form-label small fw-bold text-danger m-0 d-block">Sisa Tagihan:</label>
                        ...
                        <div class="modal-footer bg-light">
                            ...
                            <button type="submit" name="update_status" class="btn btn-sm btn-brand">Simpan</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <form method="POST" action="">
                    <div class="modal-body">
                        <div class="row g-3 mb-3">
                            <div class="col-md-6">
                                <label class="form-label small fw-bold">No. Nota</label>
                                <input type="text" name="nomor_pesanan" class="form-control font-monospace" value="<?= $nota_otomatis; ?>" readonly>
                            </div>
                            <div class="col-md-6">
                        ...
                        <div class="box-kasir mb-3">
                            <label class="form-label small fw-bold text-uppercase text-primary"><i class="fa-solid fa-plus-circle me-1"></i> Pilih Produk</label>
                            <div class="row g-2 align-items-center">
                                <div class="col-md-6">
                                    <select id="pilih_barang" class="form-select select2-search">
                                        <option value="">-- Cari Produk --</option>
                                        <?php foreach ($produk_list as $prod): ?>
                                            <option value="<?= $prod['id']; ?>" data-nama="<?= htmlspecialchars($prod['nama_produk']); ?>" data-harga="<?= $prod['harga_sewa']; ?>" data-stok="<?= $prod['stok']; ?>">
                                ...
                                <div class="col-md-3">
                                    <input type="number" id="pilih_jumlah" class="form-control form-control-sm" value="1" min="1" placeholder="Jumlah" style="height: 38px;">
                                </div>
                                <div class="col-md-3">
                                    <button type="button" class="btn btn-brand w-100 fw-bold" onclick="tambahKeKeranjang()" style="height: 38px;">
                        ...
                        <div class="table-responsive mb-3 border rounded">
                            <table class="table table-sm table-striped align-middle mb-0" id="tabelKeranjang">
                                ...
                                <tbody id="bodyKeranjang">
                                    <tr id="barisKosong"><td colspan="5" class="text-center text-muted py-3 small">Belum ada produk dipilih.</td></tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div class="row g-3 align-items-center">
                            <div class="col-md-6">
                                ...
                                <div class="mt-2 d-none" id="boxKasirDibayar">
                                    <label class="form-label small fw-bold text-success m-0">Jumlah DP (Rp)</label>
                                    <input type="number" name="sudah_dibayar" id="kasir_sudah_dibayar" class="form-control form-control-sm" value="0" min="0">
                                </div>
                            </div>
                            <div class="col-md-6 text-end">
                                <span class="d-block small text-muted">Durasi: <strong id="lblDurasi">2</strong> Hari</span>
                    ...
                    <div class="modal-footer bg-light">
                        <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">Batal</button>
                        <button type="submit" name="tambah_penyewaan" class="btn btn-sm btn-brand px-4">Simpan Transaksi</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        function hitungSisaTagihan(inputElemen) {
            ...
            if (!modal) return;
            let total = parseFloat(modal.querySelector('.val-total').value) || 0;
            let dibayar = parseFloat(inputElemen.value) || 0;
            let sisa = Math.max(0, total - dibayar);
            modal.querySelector('.lbl-sisa').innerText = "Rp " + Math.round(sisa).toLocaleString('id-ID');
        }

        function toggleBoxKalkulator(selectElemen) {
            let modal = selectElemen.closest('.modal-edit-sewa');
            if (!modal) return;
            let box = modal.querySelector('.box-kalkulator');
            let input = modal.querySelector('.val-dibayar');
            if (selectElemen.value === 'lunas') {
                box.classList.add('d-none');
            } else {
                box.classList.remove('d-none');
                if (input) hitungSisaTagihan(input);
            }
        }

        $(document).ready(function() {
            $('#modalKasirSewa').on('shown.bs.modal', function () {
                $('.select2-search').select2({ theme: 'bootstrap-5', dropdownParent: $('#modalKasirSewa') });
            });
        });
        ...
        function toggleKasirDibayar() {
            ...
            if (st === 'pending' || st === 'belum lunas') $('#boxKasirDibayar').removeClass('d-none');
            else $('#boxKasirDibayar').addClass('d-none');
        }
        ...
        function tambahKeKeranjang() {
            ...
            let opt = selectBarang.find(':selected');
            let nama = opt.data('nama');
            let harga = parseFloat(opt.data('harga')) || 0;
            let stok = parseInt(opt.data('stok')) || 0;
            let jumlah = parseInt($('#pilih_jumlah').val()) || 1;

            if (jumlah > stok) { alert("Stok tidak mencukupi!"); return; }

            let index = keranjang.findIndex(item => item.id === id);
            if (index !== -1) {
                if ((keranjang[index].jumlah + jumlah) > stok) { alert("Total melebihi stok!"); return; }
        ...
        function renderKeranjang() {
            ...
            if (keranjang.length === 0) {
                tbody.innerHTML = `<tr id="barisKosong"><td colspan="5" class="text-center text-muted py-3 small">Belum ada produk dipilih.</td></tr>`;
                hitungTotalKasir();
                return;
            }
            ...
            keranjang.forEach((item, index) => {
                ...
                tbody.innerHTML += `
                    <tr>
                        <td><strong>${item.nama}</strong><input type="hidden" name="barang_id[]" value="${item.id}"><input type="hidden" name="barang_nama[]" value="${item.nama}"></td>
                        <td>Rp ${item.harga.toLocaleString('id-ID')}</td>
                        <td><input type="hidden" name="jumlah_item[]" value="${item.jumlah}"><span class="badge bg-secondary">${item.jumlah} Unit</span></td>
                        <td class="fw-bold">Rp ${subtotal.toLocaleString('id-ID')}</td>
                        <td class="text-center"><button type="button" class="btn btn-sm btn-outline-danger py-0 px-2" onclick="hapusItemKeranjang(${index})">&times;</button></td>
                    </tr>`;
            });
            hitungTotalKasir();
        }
        ...
        function getDurasiHari() {
            ...
            let durasi = 1;
            if (tgl2 > tgl1) durasi = Math.ceil(Math.abs(tgl2 - tgl1) / (1000 * 60 * 60 * 24));
        ...
        function hitungTotalKasir() {
            let durasi = getDurasiHari();
            let grandTotal = keranjang.reduce((sum, item) => sum + (item.harga * item.jumlah * durasi), 0);
